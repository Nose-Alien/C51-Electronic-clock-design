...
void AlterTime_ShowData(void) {
    ...
    if (TimeHour_flag == 0) { // 如果当前是24小时制模式
        ...
        if (AlterRealHour == 0||AlterRealHour == 12) {// 底层0点转换为显示12 (凌晨)
            DisplayHour = 12;
            LCD_ShowString(2, 7, "AM");
        }
        else if (AlterRealHour > 12) { // 13-23点减去12，转换为下午1-11点
...
void SetAlarm_ShowData(void)
{
    ...
    if (TimeHour_flag == 0) { // 如果当前处于24小时制模式
        ...
        // 如果当前处于12小时制模式
        // 12小时制转换逻辑
        if (AlarmHour == 0||AlarmHour == 12) {// 设定为0点时，显示为12 (即凌晨12点)
            DisplayHour = 12;
            LCD_ShowString(1, 15, "AM");
            }
        else if (AlarmHour > 12){// 设定大于12点时，减去12 (即下午时间)
...
void Main_ShowData(void) {
    ...
    if (TimeHour_flag == 0) { // 如果当前是24小时制模式
        ...
        else if (RealHour > 12) { // 13-23点减去12，转换为下午1-11点
...
void ShowWeekday(unsigned int year, unsigned char month, unsigned char day)
{
    ...
    switch (weekday) {
        case 0: LCD_ShowString(1, 13, "Sun"); break;
        ...
        default: LCD_ShowString(1, 13, "Err"); break;
    }
}
