...
static unsigned int TimeMinute = 59; // 当前分钟 (0-59)
static unsigned int RealHour = 7; // 底层真实时间 (24小时制 0-23)，这是核心数据
...
void AlterTime_ShowData(void)
{
    ...
    if (TimeHour_flag == 0) {
        ...
        // 如果当前是12小时制模式
        if (AlterRealHour == 0 || AlterRealHour == 12) {
            // 底层0点转换为显示12 (凌晨)
            DisplayHour = 12;
            LCD_ShowString(2, 7, "AM");
        } else if (AlterRealHour > 12) {
            // 13-23点减去12，转换为下午1-11点
            DisplayHour = AlterRealHour - 12;
            LCD_ShowString(2, 7, "PM");
        } else DisplayHour = AlterRealHour; // 1-12点保持不变直接显示
    }

    // 2. 闪烁逻辑 (根据 AlterFlag 光标位置决定哪一位在闪烁)
    switch (AlterFlag) {
        case 1: BlinkField(2, 15, AlterSec, 2);
            break; // 光标在秒位，调用闪烁函数
        case 2: BlinkField(2, 12, AlterMinute, 2);
            break; // 光标在分位
        case 3: BlinkField(2, 9, DisplayHour, 2);
            break; // 光标在时位 (显示转换后的小时)
        case 4: BlinkField(1, 9, AlterDay, 2);
            break; // 光标在日位
        case 5: BlinkField(1, 6, AlterMonth, 2);
            break; // 光标在月位
        case 6: BlinkField(1, 1, AlterYear, 4);
            break; // 光标在年位
    }
    ...
    ShowWeekday(AlterYear, AlterMonth, AlterDay);
...
void SetAlarm_ShowData(void)
{
    ...
    if (TimeHour_flag == 0) {
        ...
        if (AlarmHour == 0 || AlarmHour == 12) {
            // 设定为0点时，显示为12 (即凌晨12点)
            DisplayHour = 12;
            LCD_ShowString(1, 15, "AM");
        } else if (AlarmHour > 12) {
            // 设定大于12点时，减去12 (即下午时间)
            DisplayHour = AlarmHour - 12;
            LCD_ShowString(1, 15, "PM");
        } else {
            DisplayHour = AlarmHour; // 1-12点保持不变，直接显示
        }
    }
...
unsigned char GetWeekday(unsigned int year, unsigned char month, unsigned char day)
{
    ...
    // 基姆拉尔森公式（系数26*(m+1)/10 等价于 13*(m+1)/5）
...
// 在主界面显示星期几（调用此函数前确保已定义好全局时间变量）
void ShowWeekday(unsigned int year, unsigned char month, unsigned char day)
{
    unsigned char weekday = GetWeekday(year, (unsigned char) month, (unsigned char) day);

    // 根据计算结果在LCD第1行第13列开始显示3个字符的星期缩写
    switch (weekday) {
        case 0: LCD_ShowString(1, 13, "Sun");
            break;
        case 1: LCD_ShowString(1, 13, "Mon");
            break;
        case 2: LCD_ShowString(1, 13, "Tue");
            break;
        case 3: LCD_ShowString(1, 13, "Wed");
            break;
        case 4: LCD_ShowString(1, 13, "Thu");
            break;
        case 5: LCD_ShowString(1, 13, "Fri");
            break;
        case 6: LCD_ShowString(1, 13, "Sat");
            break;
        default: LCD_ShowString(1, 13, "Err");
            break;
    }
}
...
void BlinkField(unsigned char row, unsigned char col, unsigned int value, unsigned char digits)
{
    ...
    if (TimeCount % 4 == 0) {
        // 利用全局定时器计数取余，控制闪烁的刷新频率（约每40ms翻转一次）
    ...
    switch (key) {
        ...
        case 4:
            if (Menu_flag == 1) {
                ...
                switch (AlterFlag) {
                    // 根据光标所在的位置进行增加
                    case 1: if (++AlterSec > 59) AlterSec = 0;
                        break; // 秒加1，超过59归零
                    case 2: if (++AlterMinute > 59) AlterMinute = 0;
                        break; // 分加1，超过59归零
                    // 修改底层小时 AlterRealHour (0-23)
                    case 3: if (++AlterRealHour > 23) AlterRealHour = 0;
                        break;
                    case 4: // 日加1
                    ...
                    case 5: // 月加1
                        ...
                        break;
                    case 6: if (++AlterYear > 9999) AlterYear = 1900;
                        break; // 年加1，超过9999回到1900
                }
                break;
            }
            if (Menu_flag == 2) {
                // 如果在闹钟设置界面
                switch (AlarmFlag) {
                    case 2: if (++AlarmMinute > 59) AlarmMinute = 0;
                        break; // 闹钟分加1
                    case 3: if (++AlarmHour > 23) AlarmHour = 0;
                        break; // 闹钟时加1
                    case 1: // 切换闹钟开关状态
                        if (AlarmOnOff == 1) {
                            AlarmOnOff = 0;
        ...
        case 7:
            if (Menu_flag == 1) {
                // 如果在时间设置界面
                switch (AlterFlag) {
                    case 1: if (AlterSec-- == 0) AlterSec = 59;
                        break; // 秒减1，到0回绕到59
                    case 2: if (AlterMinute-- == 0) AlterMinute = 59;
                        break; // 分减1，到0回绕到59
                    ...
                    case 5: // 月减1
                        ...
                        break;
                    case 6: if (AlterYear-- == 1900) AlterYear = 9999;
                        break; // 年减1，到1900回绕到9999
                }
                break;
            }
            if (Menu_flag == 2) {
